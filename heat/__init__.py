    ...
    if schedule_pattern.keep_warm and temp_actual is not None:
        ...
        if len(schedule_pattern.keep_warm_pattern) == 12:
            ...
            if delta_warm <= P.MAX_DELTA_TEMP_KEEP_WARM:
                ...
                if force_on:
                    L.l.info("Forcing heat on keep warm, zone {} interval {} pattern {}".format(
                        schedule_pattern.name, interval, schedule_pattern.keep_warm_pattern[interval]))
            else:
...
def thread_run():
    ...
    P.season = "summer" if month in range(6, 11) else "winter"
