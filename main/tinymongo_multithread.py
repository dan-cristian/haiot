...
from tinydb import Query, TinyDB, where, storages
from .results import (
    InsertOneResult,
    InsertManyResult,
    UpdateResult,
...
class TinyMongoDatabase(object):
    ...
    def __init__(self, database, foldername, storage):
        ...
        """
        self._foldername = foldername
        if issubclass(storage._storage_cls, storages.JSONStorage):
            self.tinydb = TinyDB(
...
class TinyMongoCollection(object):
    ...
    def delete_one(self, query):
        ...
        with transaction(self.table) as tr:
            result = tr.remove(where(u'_id') == item[u'_id'])
    ...
    def delete_many(self, query):
        ...
        with transaction(self.table) as tr:
            result = [
                tr.remove(where(u'_id') == item[u'_id'])
                for item in items
            ]
        return DeleteResult(raw_result=result)
