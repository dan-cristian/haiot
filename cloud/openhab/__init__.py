...
def mqtt_on_message(client, userdata, msg):
    try:
        ...
        if len(item) == 2:
            ...
            elif name.startswith("thermo_manual_target_"):
                temp = round(float(payload), 1)
                rules.thermostat(zone_name=name[len('thermo_manual_target_'):], temp_target=temp)
            ...
            elif name.startswith("areathermo_mode_manual_"):
                rules.areathermostat(area_name=name[len('areathermo_mode_manual_'):], mode_manual=switch_state)
